...
<?php include('barra.php');?>		
		...
		for($y=0;$y<count($marcado); $y++){
			...
			$qse = mysqli_query($miConex, $se) or die();
			...
			while($rowx=mysqli_fetch_array($qse)){ 
				...
				$Recordset1 = mysqli_query($miConex, $query_Recordset1) or die();
				
				$query_Recordset3 = "SELECT * FROM areas WHERE idunidades='".$rowx['idunidades']."' AND nombre='".$rowx['idarea']."' order by nombre";
				$Recordset3 = mysqli_query($miConex,$query_Recordset3) or die();
				$row_Recordset3 = mysqli_fetch_array($Recordset3);
		
				$query_Recordset2 = "SELECT * FROM tipos_medios";
				$Recordset2 = mysqli_query($miConex,$query_Recordset2) or die();
					
				$query_Recordset4 = "SELECT * FROM tipos_aft";
				$Recordset4 = mysqli_query($miConex,$query_Recordset4) or die(); 
				
				$query_Recordset5 = "SELECT * FROM sellos WHERE estado='Disponible'";
				$Recordset5 = mysqli_query($miConex,$query_Recordset5) or die();
				$damemas = mysqli_num_rows($Recordset5);
				
				$query_Recordset6 = "SELECT * FROM tipos_medios";
				$Recordset6 = mysqli_query($miConex,$query_Recordset6) or die();
				
				$query_Recordset7 = "SELECT inv FROM exp";
				$Recordset7 = mysqli_query($miConex,$query_Recordset7) or die();
				...
				<tr> 
				  <td><div align="right" class="contentheading"><?php echo $btEXPEDIENTE1;?></div></td>
				  <td colspan="2"> 
				    <input name="exp[]" id="exp[]" type="text" size="40" readonly class="form-control" value="<?php echo $rowx['exp'];?>">					
		          </td>
				</tr>
				<tr> 
				  <td colspan="3"><hr></td>
	...
	if(isset($_REQUEST['modificado'])){
		...
		foreach($idx as $key){
			...
			$seup = mysqli_query($miConex,$ft) or die();
			...
			$up ="update aft set conect='".$conectx[$x]."', enred='".$enredx[$x]."', inv='".$pinv[$x]."', descrip='".htmlentities($pdecr[$x])."', estado='".$pestado[$x]."', idarea='".htmlentities($pnombre[$x])."', sello='".$psello[$x]."', marca='".$pmarca[$x]."', no_serie='".$pserie[$x]."', modelo='".$pmodelo[$x]."', categ='".$pcateg[$x]."', tipo='".htmlentities($ptipo[$x])."', custodio='".htmlentities($pcusto[$x])."', t_AFT='".$ptaft[$x]."', exp='".$expe[$x]."', idunidades='".$tidunidades[$x]."' WHERE id='".$key."'";
			$qup =mysqli_query($miConex, $up) or die();
			
		// Busco el Talón qué está activo 
		$sqltal = "SELECT * FROM talones WHERE estado='Activo'"; 
		$resultal = mysqli_query($miConex, $sqltal) or die();
		$talonactivo =  mysqli_fetch_array($resultal);
		
		// Actualizo el estado de sello
		$sqlsello = "UPDATE sellos SET estado='En Uso', inv='".$pinv[$x]."' WHERE numero='".$psello[$x]."' AND idtalon='".$idtalon[$x]."'"; 
		$resultsello = mysqli_query($miConex, $sqlsello) or die();
	    
			// Marcar el sello viejo con Retirado
			if ($psello[$x] != $selloviejo[$x])  {
				$sqlselloviejo = "UPDATE sellos SET estado='Desechado', observ='Sustituci&oacute;n', inv='".$pinv[$x]."' WHERE numero='".$selloviejo[$x]."' AND idtalon='".$talonactivo['id']."'"; 
				$resultselloviejo = mysqli_query($miConex, $sqlselloviejo) or die();
			...
			if(($pcateg[$x])=='COMPUTADORAS'){
			      ...
			      $resultex = mysqli_query($miConex, $sqlex) or die();
				if(($pinv[$x]) !=""){
					$arreglo .= $pinv[$x]."*";
			...
			// Actualizo el estado del Talón. Si todos sus sellos han sido retirado pues inactivo el talón
				...
				$restalcie = mysqli_query($miConex, $sqltalcie) or die();
				...
				if($totalse['totalse']==0) {
				  ...
				  $resulttalonviejo = mysqli_query($miConex, $sqltalviejo) or die();
	...
	mysqli_query($miConex, $sqfk) or die();	
		foreach($marcado as $i){		
			...
			$resulte = mysqli_query ($miConex, $sql1e) or die(mysqli_error($miConex));
			...
			if(($esta) =="computadoras"){
				...
				$resultaf = mysqli_query($miConex, $sqlaf) or die(mysqli_error($miConex));
			}
			if(($esta) =="computadoras"){
			...
			$resulta = mysqli_query($miConex, $ff) or die(mysqli_error($miConex));
			if (!$resulta) {
			...
			for ($ia=1; $ia<=$num_campo;$ia++) {
				...
				if ($nom_campo == strtolower($esta))  {
					...
					$resultado1 = mysqli_query($miConex, $sql2)  or die();
					...
					$result = mysqli_query($miConex, $sql3) or die(); 
				}
			}		
		} // end for 
		$sqfk1 = "SET FOREIGN_KEY_CHECKS=1;";
		mysqli_query($miConex, $sqfk1) or die(mysqli_error($miConex));?>
			<script type="text/javascript">
