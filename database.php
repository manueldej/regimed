...
class database {
	...
	*/
	function database_u($user, $pass, $db='', $table_prefix='', $goOffline=true ) {
		...
		if (phpVersion() < '4.2.0') {
			...
			if (!($this->_resource = mysqli_connect( $host, $user, $pass, $db,'', true))) {
				$mosSystemError = 2;
				if ($goOffline) {
					$basePath = dirname( __FILE__ );
					include $basePath . '/../configuration.php';
	...
	*/
	function query() {
		...
		//$this->_cursor = mysqli_query(mysqli_connect('localhost','root', 'adminred', 'seguridad'), $this->_sql);

		// if (!$this->_cursor) {
			// $this->_errorNum = mysqli_errno($this->_cursor);
...
class mosDBTable {
	...
	*/
	function mosDBTable_u( $table, $key, &$db ) {
		$this->_tbl = $table;
		$this->_tbl_key = $key;
		$this->_db =& $db;
	}
