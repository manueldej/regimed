...
class database {
	...
	*/
	function setQuery( $sql, $offset = 0, $limit = 0, $prefix='#__' ) {
		$this->_sql = $this->replacePrefix( $prefix, $sql );
	 ...
	 */
	function replacePrefix($prefix,$sql ) {
		...
		while ($startPos < $n) {
			...
			// quote comes first, find end of quote   (YA no es aplicable a php v8.1.10)
			// while (TRUE) {
...
class mosDBTable {
	...
	*/
	function mosDBTable1( $table, $key, &$db ) {
		$this->_tbl = $table;
		$this->_tbl_key = $key;
		$this->_db =& $db;
	}
