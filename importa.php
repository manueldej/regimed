...
<script type="text/javascript">
		...
		<form name="busca" method="post" action="" enctype="multipart/form-data">
			<table width="100%" border="0" cellpadding="2" cellspacing="2" class="sgf1">
				<tr>
					<td><input title="<?php echo $arrastrar;?>" class="imputf" type="file" name="fichero" id="fichero" onblur="return LimitAttach(this.value);"/>&nbsp;&nbsp;<input type="submit" class="btn" onclick="if(document.busca.fichero.value ==''){ return false;}" name="ok" id="ok" value="<?php echo $cargar2;?>">
					</td>
				</tr>		
			</table>
			<span id="nambr"></span>
		...
		<form name="frm1" method="post" action="import_bd.php" enctype="multipart/form-data">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="sgf1"><?php
				$handle=opendir(substr($ruta,0,-1));
				$cuts=0;
				while ($file = readdir($handle)) { 
					if($file != "." && $file != ".."){ ?>
