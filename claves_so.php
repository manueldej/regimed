...
///////////
	...
	if (isset($_COOKIE['unidades']) AND ($_COOKIE['unidades']) !=""){
		$sql="SELECT * FROM reg_claves_soft where (equipo LIKE '%".$palabra."%' or login LIKE '%".$palabra."%' or usuario LIKE '%".htmlentities($palabra)."%') AND(idunidades='".$_COOKIE['unidades']."')";
	}else{
		$sql="SELECT * FROM reg_claves_soft where equipo LIKE '%".$palabra."%' or login LIKE '%".$palabra."%' or usuario LIKE '%".htmlentities($palabra)."%'";
	}
	
	$query_limit = sprintf("%s ORDER BY usuario	LIMIT %d, %d",$sql, $inicio, $registros);
	$result1=mysqli_query($query_limit,$miConex) or die(mysql_error());
			...
			<tr>
				<td align="center"><?php if ($num_resultados !=0){ ?>
						<table width="100%" border='0' align="center" cellspacing="0" class="table">
							...
							while ($row=mysqli_fetch_array($result1)){ $i++;
