...
if (isset($_POST['categ'])) {$categ = $_REQUEST['categ'];}
if (isset($_POST['marcado'])) {$marcado = $_REQUEST['marcado'];}
if (isset($_GET['inv'])) {$categ = base64_decode($_GET['inv']);}
...
if (isset($_REQUEST['idunidades']) and ($_REQUEST['idunidades']) != "") {
    $query = "select * from bajas_aft where inv='" . @$categ . "' AND idunidades='" . $idunidadesc . "'";
} else {
    $query = "select * from bajas_aft where inv='" . @$categ . "'";
}
...
if (isset($_COOKIE['unidades']) and ($_COOKIE['unidades']) != "") {
    $sql1 = "SELECT * FROM `bajas_exp` where inv='" . @$categ . "' and idunidades='" . $_COOKIE['unidades'] . "'";
} elseif (isset($_REQUEST['idunidades']) and ($_REQUEST['idunidades']) != "") {
    $sql1 = "SELECT * FROM `bajas_exp` where inv='" . @$categ . "' and idunidades='" . $_REQUEST['idunidades'] . "'";
} else {
    $sql1 = "SELECT * FROM `bajas_exp` where inv='" . @$categ . "'";
}
...
$sql23 = "SELECT * FROM bajas_exp WHERE inv ='" . $categ . "'";
...
<script language="JavaScript" >
 ...
 function ocultacat(tipo){
	...
	function __importa(inv,marcado,idunidades) {
	  document.creabajas_exp.inv.value = inv;
	  document.creabajas_exp.marcado.value = marcado;
	  document.creabajas_exp.idunidades.value = idunidades;
	  document.creabajas_exp.impor.value = 'kbpimpirt';
	  document.creabajas_exp.submit();
	}

	function __creabajas_expt(inv,marcado,idunidades) {
	  document.creabajas_exp.inv.value = inv;
	  document.creabajas_exp.marcado.value = marcado;
	  document.creabajas_exp.idunidades.value = idunidades;
	  document.creabajas_exp.submit();
...
if (($num_resultados1) != 0) {?>
	    <tr>
		   <td colspan="2"><img src="images/cabeceraET.jpg" width="100%" height="75">
				...
				<div id="imprime">
				  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr><?php
					  ...
					  <td class="exel"><a href="expexp.php?query=<?php echo $ggg; ?>&tb=bajas_exp&categ=<?php echo $categ; ?>" target="_blank" class="tooltip">&nbsp;&nbsp;&nbsp;<span onMouseOver="this.style.cursor='pointer';" ><?php echo strtoupper($cr_exel); ?></span></a></td>
                      <td class="printer"><a style="cursor:pointer;" onclick="__print('<?php echo $categ; ?>','bajas_exp');" target="_blank" class="tooltip">&nbsp;&nbsp;&nbsp;<span onMouseOver="this.style.cursor='pointer';" ><?php echo strtoupper($sav_print); ?></span></a></td>
                    </tr>
                  </table>
				</div>
			</td>
...
$sql = "SELECT * FROM bajas_exp WHERE custodio='" . @$custo . "' AND inv ='" . @$categ . "' and idunidades='" . @$idunidas . "'";
...
while ($rowa = mysqli_fetch_array($resulaft)) {?>
				...
				<form name="etc" id="etc" method="post" action="modificarbajas_exp.php">
...
if (($num_resultados1) != 0) {
    if (($palabra) != "") {?>
							<input name="bajas_expediente" type="button" id="bajas_expediente" value="<?php echo $btcancelar; ?>" class="btn" onclick="document.location='registromedios1.php?palabra=<?php echo $palabra; ?>';"><?php
} elseif ((@$nom_custo)) {?>
							<input name="bajas_expediente" type="button" id="bajas_expediente" value="<?php echo $btcancelar; ?>" class="btn" onclick="document.location='registrocustodio.php?nom_custo=<?php echo $nom_custo; ?>';"><?php
} elseif (($dde) == "rm") {?>
							<input name="bajas_expediente" type="button" id="bajas_expediente" value="<?php echo $btaceptar; ?>" class="btn" onclick="document.location='registromedios1.php?otras=&palabra=';"><?php
} else {?>
							<input name="bajas_expediente" type="submit" id="bajas_expediente" value="<?php echo $btaceptar; ?>" class="btn"> <?php
}?>
