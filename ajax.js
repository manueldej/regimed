...
function verareas(valo){
	divar = document.getElementById('buscaarea');
	ajaxarea=objetoAjax();
	ajaxarea.onreadystatechange=function() {
		if (ajaxarea.readyState==4) {
			divar.innerHTML=ajaxarea.responseText;
			var datos=ajaxarea.responseXML.documentElement;	
		}		
	}
	document.getElementById('idunidades').value = valo;
	valorarea="idunidades="+valo;
	ajaxarea.open("POST", "buscaarea.php", true);
	ajaxarea.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
	ajaxarea.send(valorarea);
}
function verusuarios(valo,idunidades){
	divusuarios = document.getElementById('vistauser');
	ajaxusuarios=objetoAjax();
	ajaxusuarios.onreadystatechange=function() {
		if (ajaxusuarios.readyState==4) {
			divusuarios.innerHTML=ajaxusuarios.responseText;
			var datos=ajaxusuarios.responseXML.documentElement;	
		}		
	}
	document.getElementById('narea').value = valo;
	document.getElementById('idunid').value = idunidades;
	valoruser="tt="+valo+"&idunidades="+idunidades;
	ajaxusuarios.open("POST", "buscauser.php", true);
	ajaxusuarios.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
	ajaxusuarios.send(valoruser);
}
...
function ubicar_origen(valo){
	divinventx = document.getElementById('noinv');
	ajaxnoinv=objetoAjax();
	ajaxnoinv.onreadystatechange=function() {
		if (ajaxnoinv.readyState==4) {
			divinventx.innerHTML=ajaxnoinv.responseText;
			var datoorigen=ajaxnoinv.responseXML.documentElement;	
		}		
	}
    
	if(valo ==-1){
	   document.getElementById('inventax').value="-1";
	}
...
function u_destino(valo){
	if(valo!=-1){
	   document.getElementById('udestino_f').style.display="block";
	   document.getElementById('udestino_final').style.display="block";
	}else{
	   document.getElementById('udestino_f').style.display="none";
...
function dame_custodio(valo,uni,af){
	if(valo=="UmVwYXJhY2lvbmVz"){
		document.getElementById('divcustodio').style.display ='none';
		document.getElementById('labelcustodio').style.display ='none';
	}else{
		document.getElementById('divcustodio').style.display ='block';
		document.getElementById('labelcustodio').style.display ='block';
	}
