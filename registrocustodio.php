...
///////navegador
	...
	$row1 = mysqli_fetch_array($resultado, MYSQL_ASSOC);
	$nom_custo = "all"; 
...
<body onload="if (document.getElementById('traspa').checked ==true ) { document.getElementById('cheq').style.visibility=""; }else{ document.getElementById('cheq').style.visibility='hidden'; }">
<?php
$aftt = base64_encode($sql);
 include('barra.php');?>
<div id="buscad"> 
<fieldset class="fieldset"><legend class="vistauserx"><?php echo $registr5.$de.$btCustodios1;?><?php if(($unidadactiva) !=""){ echo "&nbsp;&nbsp;&nbsp;".$btdatosentidad3.": <font color='red'>".$unidadactiva."</font>"; }?></legend><?php
		...
		<table width="100%" height="122" border="0" align="center">
		  ...
		  <tr>
			...
			<td width="12%"><?php
				if(($total_registros) !=0){?>
					<form id="form2" name="form2" method="post" action="">
			  			...
			  			<input name="traspa" style="cursor:pointer;" type="checkbox" id="traspa"  value="traspa"  <?php   if(isset($_GET['adestino'])){ echo "checked"; }?>  onClick="if((this.checked) ==false){ cheqec('f'); }else{ cheqec('t'); document.getElementById('labelareas').style.display='block';  document.getElementById('comboareas').style.display='block'; }"  />
						...
						<select class="combo_box" name="area" size="1" id="area" onchange=" if((this.value) =='-1'){ 	document.getElementById('usuar').style.display='none'; document.getElementById('labelus').style.display='none';}">
							...
							while ($row_R = mysqli_fetch_array($Record)) {  
								if((@$rse['idarea']) !=$row_R['nombre']){	?>
                  					<option onclick=" envia('<?php echo base64_encode($row_R['idarea'])?>','<?php echo @base64_encode($nom_custo);?>','<?php echo $registros;?>');" value="<?php echo $row_R['idarea'];?>"<?php if(($row_R['idarea']) ==@$adestn){echo "selected";}?>><?php echo $row_R['nombre'];?></option><?php
								}
							} ?>
           			  </select>
				  </div>
	        ...
	        <td width="25%"><span id="labelus" <?php if(isset($_GET['adestino'])){ ?>style="display:block; text-align:right;" <?php }else{ ?>style="display:none; text-align:right;"<?php } ?>><b><?php echo strtoupper($btusuario); ?></b></span></td>
		        <td width="21%"><div id="usuar"  <?php if(isset($_GET['adestino'])){ ?>style="display:block;" <?php }else{ ?>style="display:none;"<?php } ?>>
                  <select class="combo_box" name="custodio" size="1" id="custodio" onchange="if((this.value) !='-1'){ muestraboton('s','<?php echo base64_decode($_GET['adestino']);?>'); document.getElementById('custodiod').value=this.value;}else{ muestraboton('n',''); }">
                    ...
                    <option value="<?php echo $row_Recordset4['nombre']?>" <?php if(isset($_GET['custo'])){ if((base64_decode($_GET['custo'])) ==$row_Recordset4['nombre']){ echo "selected"; }	}?>><?php echo $row_Recordset4['nombre']?></option>
                    <?php
										} ?>
                  </select>
                </div></td>
			...
			<form action="trasp1.php" method="post" enctype="multipart/form-data" name="frm1"> 
				<table width="100%" BORDER="0" class="table" align="center" cellpadding="0" cellspacing="0">
					...
					while($row=mysqli_fetch_array($result)) { $i++; ?>
						...
						<td>
                           <?php if (($russ['tipo']) =="root") { ?>
                            <span id="che<?php echo $p;?>" style="visibility:hidden;"><div id="chequeadera<?php echo $p;?>" style="background:url(gfx/checkbox.gif) no-repeat scroll 0 -15px transparent;">&nbsp;&nbsp;&nbsp;&nbsp;</div><input name="marcado[]" type="checkbox" style="display:none; cursor:pointer;" id="marcado<?php echo $p;?>" onClick="marca1('<?php echo $p;?>','#ffffff')" value="<?php echo $row["id"]; ?>" /></span><?php
						} ?>
                                                </td>
                                                <td>
                                                <?php 
						...
						<td><div align="justify">
						    <?php  echo $row["inv"];?>
						    </div><input name="invent1[]" type="hidden" value="<?php echo $row["inv"];?>">
                                                </td>
