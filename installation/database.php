...
class database {
	...
	*/
	function database( $host='localhost', $user, $pass, $db='', $table_prefix='', $goOffline=true ) {
        ...
        //@mysqli_query($miConex, "SET NAMES 'utf8'", $this->_resource);
		$this->_ticker = 0;
		$this->_log = array();
	}
	/**
	...
	*/
	function query() {
		...
		$this->_errorNum = 0;
		$this->_errorMsg = '';
		$this->_cursor = mysqli_query($miConex,  $this->_sql, $this->_resource );
		if (!$this->_cursor) {
	...
	function query_batch( $abort_on_error=true, $p_transaction_safe = false) {
		...
		foreach ($query_split as $command_line) {
			...
			if ($command_line != '') {
				$this->_cursor = mysqli_query($miConex,  $command_line, $this->_resource );
				if (!$this->_cursor) {
					$error = 1;
					$this->_errorNum .= mysql_errno( $this->_resource ) . ' ';
