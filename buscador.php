	...
	if(($rsel['visitas']) !=""){
		...
		///////navegador
				...
				if(($palabra) !=""){ 
					...
					$rbuscar_CPU = mysql_fetch_assoc($qbuscar_CPU);
					...
					$rbuscar_PLACA = mysql_fetch_assoc($qbuscar_PLACA);
					...
					$rbuscar_MEMORIA = mysql_fetch_assoc($qbuscar_MEMORIA);
					...
					$rbuscar_MEMORIA2 = mysql_fetch_assoc($qbuscar_MEMORIA2);
					...
					$rbuscar_GRAFICS = mysql_fetch_assoc($qbuscar_GRAFICS);
					...
					$rbuscar_DRIVE1 = mysql_fetch_assoc($qbuscar_DRIVE1);
					...
					$rbuscar_DRIVE2 = mysql_fetch_assoc($qbuscar_DRIVE2);
					...
					$rbuscar_DRIVE3 = mysql_fetch_assoc($qbuscar_DRIVE3);
					...
					$rbuscar_DRIVE4 = mysql_fetch_assoc($qbuscar_DRIVE4);
					...
					$rbuscar_RED = mysql_fetch_assoc($qbuscar_RED);
					...
					$rbuscar_RED2 = mysql_fetch_assoc($qbuscar_RED2);
					...
					$rbuscar_doct = mysql_fetch_assoc($qbuscar_doct);
					...
					$rbuscar_docono = mysql_fetch_assoc($qbuscar_docono);
					...
					$rbuscar_doc = mysql_fetch_assoc($qbuscar_doc);
					...
					$rbuscar_font = mysql_fetch_assoc($qbuscar_font);
					...
					$rbuscar_sert = mysql_fetch_assoc($qbuscar_sert);
				...
				//	
					...
					$rbuscar_ser = mysql_fetch_assoc($qbuscar_ser);
				...
				//
					...
					$rbuscar_fon = mysql_fetch_assoc($qbuscar_fon);
				...
				//
					...
					$rbuscar_ini = mysql_fetch_assoc($qbuscar_ini);
				...
				//
					...
					$rbuscar_recoger = mysql_fetch_assoc($qbuscar_recoger);
				...
				//
					...
					$rbuscar_conservar = mysql_fetch_assoc($qbuscar_conservar);
				...
				//
					...
					$rbuscar_servir = mysql_fetch_assoc($qbuscar_servir);
				...
				//
					...
					$rbuscar_arti = mysql_fetch_assoc($qbuscar_arti);
				...
				//
					...
					$rbuscar_novedades = mysql_fetch_assoc($qbuscar_novedades);
				...
				//
					...
					$rbuscar_creditos = mysql_fetch_assoc($qbuscar_creditos);
				...
				//
					...
					$rbuscar_resolT = mysql_fetch_assoc($qbuscar_resolT);
				...
				//
					...
					$rbuscar_resolD = mysql_fetch_assoc($qbuscar_resolD);
					$tbuscar_resolD = mysqli_num_rows($qbuscar_resolD);
					$size = strlen ($palabra);
					
					if (($tbuscar_doct) ==0 AND ($tbuscar_CPU) ==0 AND ($tbuscar_MEMORIA) ==0 AND ($tbuscar_MEMORIA2) ==0 AND ($tbuscar_PLACA) ==0 AND ($tbuscar_GRAFICS) ==0 AND ($tbuscar_DRIVE1) ==0 AND ($tbuscar_DRIVE2) ==0 AND ($tbuscar_DRIVE3) ==0 AND ($tbuscar_DRIVE4) ==0 AND ($tbuscar_RED) ==0 AND ($tbuscar_RED2) ==0 AND ($tbuscar_docono) ==0 AND ($tbuscar_doc) ==0 AND ($tbuscar_font) ==0 AND ($tbuscar_sert) ==0 AND ($tbuscar_ser) ==0 AND ($tbuscar_fon) ==0 AND (@$tbuscar_ini) ==0 AND ($tbuscar_recoger) ==0 AND ($tbuscar_conservar) ==0 AND ($tbuscar_servir) ==0 AND ($tbuscar_arti) ==0 AND ($tbuscar_novedades) ==0 AND ($tbuscar_creditos) ==0 AND ($tbuscar_resolD) ==0 AND ($tbuscar_resolT) ==0){ 
							$meng= $no_result;
					} else {
							...
							if(($rbuscar_doct['inv']) !=""){
								$sql = "insert into buscador values (NULL,'".$palabra."','".$palabra."','detalleexp.php?registros=5&palabra=".$palabra."','Activo Fijo--> (Invent)&nbsp;&nbsp;&nbsp;&nbsp;<font color=#BFCBD2>".$coincidencia.$tbuscar_doct."</font>')";
								mysqli_query($miConex, $sql) or die(mysql_error());
							}
							//
						...
						//
						while($rbuscar_fon = mysql_fetch_assoc($qbuscar_fon)){
						...
						//
						$busqini =$rbuscar_ini['idarea'];
						$dominioini = stristr ($busqini, $palabra);
						$restoini = substr ($dominioini, $size,40);
						if(($rbuscar_ini['idarea']) !=""){
							$sql = "insert into buscador values (NULL,'".$palabra."','".$palabra."','expt.php?registros=5&palabra=".$palabra."','Expediente--> (&Aacute;rea)&nbsp;&nbsp;&nbsp;&nbsp;<font color=#BFCBD2>".$coincidencia.$tbuscar_ini."</font>')";
							mysqli_query($miConex, $sql) or die(mysql_error());
						}
						//
						...
						//
						while($rbuscar_recoger = mysql_fetch_assoc($qbuscar_recoger)){
							$busqrecoger =$rbuscar_recoger['inv'];
							$dominiorecoger = stristr ($busqrecoger, $palabra);
							$restorecoger = substr ($dominiorecoger, $size,40);
							if(($rbuscar_recoger['inv']) !=""){
								$sql = "insert into buscador values (NULL,'".$palabra."','".$palabra."','expt.php?registros=5&palabra=".$palabra."','Expediente--> (Invent)&nbsp;&nbsp;&nbsp;&nbsp;<font color=#BFCBD2>".$coincidencia.$tbuscar_recoger."</font>')";
								mysqli_query($miConex, $sql) or die(mysql_error());
							}
						}
						//
						$busqservir =$rbuscar_servir['custodio'];
						$dominioservir = stristr ($busqservir, $palabra);
						$restoservir = substr ($dominioservir, $size,40);
						if((@$rbuscar_servir['custodio']) !=""){
							$sql = "insert into buscador values (NULL,'".$palabra."','".$palabra."','expt.php?registros=5&palabra=".$palabra."','Expediente--> (Custodio)&nbsp;&nbsp;&nbsp;&nbsp;<font color=#BFCBD2>".$coincidencia.$tbuscar_servir."</font>')";
							mysqli_query($miConex, $sql) or die(mysql_error());
						}
						//
					...
					$p=0;
					while($rbus=mysql_fetch_assoc($result)){ $p++;?>
